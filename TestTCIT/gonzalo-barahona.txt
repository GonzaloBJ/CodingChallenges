...
function sortPaddockTypeByTotalArea() {
    return paddockType.map((paddockType) => {
        return {
            name: paddockType.name,
            totalArea: paddocks.filter((paddock) => paddock.paddockTypeId === paddockType.id).reduce((accumulator, currentValue) => accumulator + currentValue.area, 0)
        }
    }).sort((a, b) => {
        if (a.totalArea > b.totalArea) return -1;
    }).map((paddockTotalArea) => paddockTotalArea.name);
}
...
function sortFarmManagerByAdminArea() {
    return paddockManagers.map((paddockManager) => {
        return {
            ...
            totalPaddocks: paddocks.filter((paddock) => paddock.paddockManagerId === paddockManager.id).length
        }
    }).sort((a, b) => {
        if (a.totalPaddocks > b.totalPaddocks) return -1;
    }).map((paddockManagerCount) => paddockManagerCount.name);
}
...
function farmManagerNames() {
    const farmManagers = farms.map((farm) => {
        const managersByFarm = [...new Set (paddocks.filter((paddock) => paddock.farmId === farm.id)
            .map((farmPaddock) => paddockManagers.filter((manager) => manager.id === farmPaddock.paddockManagerId).pop()))];

        const managersTax = [...managersByFarm.sort((a, b) => {
            if (a.name < b.name) return -1;
        }).map((managerFarm) => managerFarm.taxNumber)];
          
        return [farm.name, managersTax]

    })
    let result = {};
    farmManagers.forEach((farm)=> {
        result = {
            ...result,
            [farm[0]] : farm[1]
        }
...
function biggestAvocadoFarms() {
    const m2Limit = 20000;
    const paltosId = paddockType.filter((paddock) => paddock.name === 'PALTOS').pop().id;

    return farms.map((farm) => {
        const paltosPaddocks = paddocks.filter((paddock) => paddock.paddockTypeId === paltosId && paddock.farmId === farm.id);

        const m2ByPaltosFarm = paltosPaddocks.reduce((accumulator, currentValue) => accumulator + currentValue.area, 0);

        return m2ByPaltosFarm > m2Limit? m2ByPaltosFarm : null;
    }).sort((a, b) => {
        if (a > b) return -1;
    }).filter((farm) => farm );
}
...
function biggestCherriesManagers() {
    const farmId = farms.filter((farm) => farm.name === 'FORESTAL Y AGRICOLA LO ENCINA').pop().id;
    const m2Limit = 1000;
    const cerezasId = paddockType.filter((type) => type.name === 'CEREZAS').pop().id;

    return paddockManagers.map((manager) => {
        const cerezasPaddocks = paddocks.filter((paddock) => paddock.paddockManagerId === manager.id && paddock.paddockTypeId === cerezasId && paddock.farmId === farmId);

        const cerezasM2ByManager = cerezasPaddocks.reduce((accumulator, currentValue) => accumulator + currentValue.area, 0);

        return cerezasM2ByManager > m2Limit ? manager.name : null;
    }).sort((a, b) => {
        if (a > b) return -1;
...
function farmManagerPaddocks() {
    const farmManagers = paddockManagers.map((manager) => {
        const farmsByManager = [...new Set (paddocks.filter((paddock) => paddock.paddockManagerId === manager.id)
            .map((managerPaddock) => farms.filter((farm) => farm.id === managerPaddock.farmId).pop()))];

        const farmsNames = [...farmsByManager.sort((a, b) => {
            if (a.name < b.name) return -1;
        }).map((farm) => farm.name)];
          
        return [manager.name, farmsNames]
...
function paddocksManagers() {
    const typeByYearManager = paddocks.map((paddock) => {
        const typeName = paddockType.filter((type) => type.id === paddock.paddockTypeId).pop().name;
        const typeByYear = `${typeName}-${paddock.harvestYear}`;

        const managerByType = paddockManagers.filter((manager) => manager.id === paddock.paddockManagerId).pop();

        return [typeByYear, managerByType];
    })

    let result = {}; 
    typeByYearManager.forEach((typeByYear) => {
        result = {
            ...result,
            [typeByYear[0]]: { [typeByYear[1].id]:  typeByYear[1].name}
        }
    })
...
function newManagerRanking() {
    const agricolaId = farms.filter((farm) => farm.name === 'AGRICOLA SANTA ANA' ).pop().id;
    const nogalesId = paddockType.filter((type) => type.name === 'NOGALES' ).pop().id;

    let inmutablePaddocks = paddocks;
    let inmutablePaddockManagers = paddockManagers;

    const newManager = { id: 7, taxNumber: '18331219', name: 'GONZALO BARAHONA JIMÉNEZ' };
    inmutablePaddockManagers.push(newManager);

    const newPaddock = {paddockManagerId: 7, farmId: agricolaId, paddockTypeId: nogalesId, harvestYear: 2017, area: 900 };
    inmutablePaddocks.push(newPaddock);

    const ranking = inmutablePaddockManagers.map((paddockManager) => {
        return {
            name: paddockManager.name,
            totalPaddocks: paddocks.filter((paddock) => paddock.paddockManagerId === paddockManager.id).length
        }
    ...
    return ranking.indexOf(newManager.name)+1;
}
